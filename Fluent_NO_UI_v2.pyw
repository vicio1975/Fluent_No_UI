...
The code allows to run Flunet in background mode
"""
...
frame00.grid(row=1, column=0,pady=1,ipady=8, padx = 5 , ipadx = 20)
frame00.config(borderwidth=2, bg = '#AEBEDC', relief="ridge")
#flat, groove, raised, ridge, solid, or sunken

frame22 = tk.LabelFrame(top_frame, width=20, height=20, text="Fluent Version",font=f_H12B)
frame22.grid(row=0, column=0,pady=10,ipady=3, ipadx = 11)
frame22.config(borderwidth=2, bg = '#AEBEDC')

frame33 = tk.LabelFrame(top_frame, width=10, height=50, text="Solver type",font=f_H12B)
frame33.grid(row=4, column=0, padx=5, pady=8, ipadx=20, ipady=1)
frame33.config(borderwidth=2, bg = '#AEBEDC')
...
def ACTF():
    ...
    """
    if ds.get()==1:
       ds_1.configure(fg='black',font=f_H12B)
       ds_2.configure(fg='gray',font=f_H12)
    if ds.get()==2:
        ds_1.configure(fg='gray',font=f_H12)
        ds_2.configure(fg='black',font=f_H12B)
...
sd.pack(expand=True)
sd.insert('end', "1")
sd.configure(state='normal', bg='#FFFFFF')

#2d or 3d button solver
ds = tk.IntVar()
ds_1 = tk.Radiobutton(frame33, text = "2ddp", variable = ds,
                 value = 1, height=1, width = 6, font=f_H12,fg='gray')
ds_1.pack(side='bottom', fill='x', ipadx=1, padx=5, pady=2)
ds_1.configure(bg='#AEBEDC')
ds_1.configure(command=ACTF,indicatoron=1)

ds_2 = tk.Radiobutton(frame33, text = "3ddp", variable = ds,
                 value = 2, height=1, width = 6, font=f_H12B)
ds_2.pack(side='top', fill='x', ipadx=1, padx=5, pady=2)
ds_2.configure(command=ACTF, indicatoron=1, bg='#AEBEDC')
ds_2.select()

##############Buttons
#Journal Button
journal_btn = tk.Button(frame00, text="Open Journal",
                        command =open_j, font=f_H12, bg='#FFFFFF')
journal_btn.pack(expand=True)

#Run button
run_butt = tk.Button(frame11, text="Multiple-runs", font=f_H12, command=threading_1,
                     state=tk.DISABLED)
run_butt.config(height=1, width=10,  bg='#FFFFFF')
run_butt.pack(side='left', fill='x', ipadx=2, padx=3, pady=5)

#Graph button
gr_butt = tk.Button(frame11, text="GRAPH", font=f_H12, command=threading_2,
                     state=tk.DISABLED)
gr_butt.config(height=1, width=10, bg='#AEBEDC')
gr_butt.pack(side='left', fill='x', ipadx=2, padx=3, pady=5)

#Exit Button
ex = tk.Button(frame12, text="EXIT", command=ex, font=f_H12)
ex.config(height=1, width=10 ,bg='#FFFFFF')
